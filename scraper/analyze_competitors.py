...
def analyze_competitor_data():
    ...
    try:
        ...
        print(f"Competitors with phone numbers: {len(df[df['phone'] != 'N/A'])}")
        print(f"Competitors with service areas: {len(df[df['service_areas'] != 'N/A'])}")
        print(f"Competitors with pricing info: {len(df[df['pricing_info'] != 'N/A'])}\n")
        ...
        pricing_data = df[df['pricing_info'] != 'N/A']
        if len(pricing_data) > 0:
            ...
            for _, row in pricing_data.iterrows():
                ...
                pricing = row['pricing_info'][:100] + "..." if len(row['pricing_info']) > 100 else row['pricing_info']
        ...
        industries_with_phones = df[df['phone'] != 'N/A']['industry'].unique()
        ...
        industries_with_pricing = df[df['pricing_info'] != 'N/A']['industry'].unique()
